...
public class StaticMaps extends MapsJava {
     ...
     * @see Maptype
     */
    public Image getStaticMap(String centerAddress,int zoom,int alt, int anch,int scale,Format format, Maptype maptype) throws MalformedURLException, UnsupportedEncodingException, FileNotFoundException {
        URL url=new URL(URLRoot + "?center=" + URLEncoder.encode(centerAddress, "utf-8") + "&zoom=" + zoom +
                "&size=" + alt + "x" + anch + "&scale=" + scale +
        ...
        try {
            Image imageReturn = new Image(""+url);
            return imageReturn;
        } catch (Exception e) {
